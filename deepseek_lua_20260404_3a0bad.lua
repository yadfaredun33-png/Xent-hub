...
-- ============================================
-- GUI SETUP - SMALLER UNIFIED PANEL
-- ============================================
...
shadow.Size = UDim2.new(0, 340, 0, 480)
shadow.Position = UDim2.new(0.5, -170, 0.5, -240)
...
task.spawn(function()
    while screenGui and mainFrame and mainFrame.Parent do
        task.wait(0.05)
        local alpha = 0.2 + (math.abs(math.sin(tick() * 1.5)) * 0.5)
        mainStroke.Transparency = 1 - alpha
        local colorVal = 100 + math.floor(55 * (math.sin(tick() * 1.5) + 1) / 2)
        mainStroke.Color = Color3.fromRGB(0, 100 + math.floor(colorVal * 0.5), 255)
    end
end)
...
local leftRow, leftBtn = createToggleRow(ragCard, "AUTO TP LEFT", true, false, function(on)
    autoTpLeft = on
    if on then 
        autoTpRight = false
        if rightRow then
...
local rightRow, rightBtn = createToggleRow(ragCard, "AUTO TP RIGHT", false, false, function(on)
    autoTpRight = on
    if on then 
        autoTpLeft = false
        if leftRow then
...
local normalSize = UDim2.new(0, 320, 0, 460)
local miniSize = UDim2.new(0, 320, 0, 45)

local function setMinimized(state)
    minimized = state
    if minimized then
        TweenService:Create(mainFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {Size = miniSize, Position = UDim2.new(0.5, -160, 0.5, -22)}):Play()
        TweenService:Create(shadow, TweenInfo.new(0.25), {Size = UDim2.new(0, 340, 0, 65), Position = UDim2.new(0.5, -170, 0.5, -32)}):Play()
        scrollFrame.Visible = false
        minimizeBtn.Text = "□"
    else
        TweenService:Create(mainFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {Size = normalSize, Position = UDim2.new(0.5, -160, 0.5, -230)}):Play()
        TweenService:Create(shadow, TweenInfo.new(0.25), {Size = UDim2.new(0, 340, 0, 480), Position = UDim2.new(0.5, -170, 0.5, -240)}):Play()
