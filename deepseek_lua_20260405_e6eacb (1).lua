...
-- MOBILE FIXED: Drag without moving camera, Working sliders
...
-- ============================================
-- GUI - MOBILE FRIENDLY WITH FIXED DRAG
-- ============================================
local isMobile = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled
local guiSize = isMobile and UDim2.new(0, 300, 0, 450) or UDim2.new(0, 330, 0, 490)
...
task.spawn(function()
    while screenGui and mainFrame do
        ...
        border.Color = Color3.fromRGB(0, g, b)
        border.Transparency = 0.2 + math.sin(t) * 0.2
    end
end)

-- TOP BAR (ONLY DRAGGABLE AREA)
...
-- DRAG FUNCTIONALITY - PREVENTS CAMERA MOVEMENT ON MOBILE
...
topBar.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        -- Prevent touch from passing through to the game on mobile
...
local function makeSlider(parent, labelText, minVal, maxVal, defaultValue, callback)
    ...
    container.Size = UDim2.new(1, -10, 0, isMobile and 65 or 70)
    ...
    -- Make the slider button larger for mobile touch
    ...
    -- For mobile touch
    sliderBtn.TouchBegan:Connect(function() dragging = true end)
    sliderBtn.TouchEnded:Connect(function() dragging = false end)
    ...
    -- For mobile touch movement
    UserInputService.TouchMoved:Connect(function(touch)
        if dragging then
            local rel = touch.Position.X - sliderBg.AbsolutePosition.X
            local pct = math.clamp(rel / sliderBg.AbsoluteSize.X, 0, 1)
            local val = math.floor(minVal + pct * (maxVal - minVal))
            fill.Size = UDim2.new(pct, 0, 1, 0)
            label.Text = labelText .. val
...
print("XENTHUB DUELS FULLY LOADED - Drag only works on top bar | Sliders work on mobile")